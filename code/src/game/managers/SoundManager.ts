class SoundManager {
	...
	public set musicVolume(value) {
		let beforeValue = this._musicVolume;
	...
	public playMusic(musicName: string, loop = true) {
		...
		if (musicName) {
			...
			this.musicSource = musicName;
			this.musicLoop = loop;
			let musicRes = RES.getRes(musicName);
			if (musicRes) {
				this.music = new Howl({
					src: [musicRes.url],
					loop: loop,
				...
				RES.getResAsync(musicName, (musicRes, t) => {
					this.musicSource = null;
					t == this.musicSource && this.playMusic(this.musicSource, this.musicLoop)
				}, this)
			}
			musicRes = null;
		}
