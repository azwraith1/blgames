    ...
    export function addPopUp(panel, dark: boolean = false, popUpWidth: number = 0, popUpHeight: number = 0, effectType: number = 0, isAlert: boolean = false): void {
        ...
        switch (effectType) {
            ...
            case 6:
                ...
                break;
            case 66:
                 (panel as egret.DisplayObject).anchorOffsetX = panel.width / 2;
                (panel as egret.DisplayObject).anchorOffsetY = panel.height / 2;
                panel.alpha = 0;
                panel.scaleX = 0;
                panel.scaleY = 0; 
                panel.x = panel.width/2;
                panel.y = panel.height/2;
                egret.Tween.get(panel).to({scaleX: 1, scaleY: 1}, 250, egret.Ease.cubicOut);
                egret.Tween.get(panel).to({alpha:1},200,egret.Ease.cubicOut);
                break;
        ...
        return new Promise(function (resolve, reject) {
            ...
            switch (effectType) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    panel.anchorOffsetX = panel.x + panel.width / 2;
                    panel.anchorOffsetY = panel.height / 2;
                    egret.Tween.get(panel).to({ scaleX: 0.8, scaleY: 0.8 }, 150).to({ alpha: 0, scaleX: 0, scaleY: 0 }, 200);
                    break;
                default:
                    break;
            }
