...
module majiang {
	 ...
	 */
		...
		//亮倒
		public liangdaoMJ(playerData: PlayerGameDataBean) {
			let displayCard = playerData.displayCard;
			let displayCards = MajiangUtils.getCardArrByJson(displayCard);
			//暗牌长度
			let maxCard = [14, 11, 8, 5, 2];
			let startLength = playerData.cardNum;
			if (maxCard.indexOf(startLength) > -1) {
				startLength -= 1;
			}
			let length = startLength - displayCards.length;
			for (let i = 38; i > 38 - length; i--) {
				this[`image${i}`].visible = true;
			}
			//显示明牌长度
