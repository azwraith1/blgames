...
module majiang {
	...
	export class MajiangHupai extends game.BaseUI {
		...
		private showMaimaiWinAni() {
			let db = new DBComponent("gdmj_mapai");
			db.x = 30;
			db.y = 30;
			db.callback = () => {
		...
		public setMaiMaMaskColor(state: number) {
			if (state == 1) {
		...
		public showHouseValue(value) {
			this.colorImage.visible = true;
			this.bgImage.source = this.bgImage1.source;
			let color = this.colorStr.replace("`VALUE`", value + "");
			this.colorImage.source = RES.getRes(color);
		}
