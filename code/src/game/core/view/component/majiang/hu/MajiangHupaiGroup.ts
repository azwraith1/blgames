module majiang {
	export class MajiangHupaiGroup extends game.BaseUI {
		...
		public addHousePai() {
			let hupai = this.addBuhua(-1) as MajiangHupai;
			hupai.change2bei();
		}

		public showHouseValue(value, state) {
			let pai = this.majiangs[0];
			pai.showHouseValue(value);
			pai.setMaiMaMaskColor(state);
		}

		public addBuhua(card) {
			let hupai = new MajiangHupai(this.direction, card);
		...
		public addHu(huJson, type: number = 0) {
			...
			if (huJson.mainCard == true) { hupai.alpha = 0.7; }
			let numberIndex = this.majiangs.length;
			let row = Math.floor(numberIndex / 3);
			let cow = numberIndex % 3;
			hupai.index = numberIndex + 1;
