module majiang {
	export class HZMJMineShoupaiGroup extends BaseShoupaiGroup {
		public shoupais: HZMJMineShoupai[] = [];
		public mopai: HZMJMineShoupai;
		public hupaiMap;
		public lastTouchPai: HZMJMineShoupai;
		...
		public shoupaiTouchOn(e: egret.TouchEvent) {
			...
			this.lastIndex = -1;
			for (let i = 0; i <= 14; i++) {
				var majiang = this.mainGroup.getChildByName("mj" + (i + 1)) as HZMJMineShoupai;
		...
		public initWithArr(majiangArr, visible: boolean = true) {
			...
			this.mopai = new HZMJMineShoupai(0);
		...
		private createShoupai(index, value): HZMJMineShoupai {
			var shoupai = new HZMJMineShoupai(parseInt(value));
		...
		public changeShoupaiValue(name, value) {
			let shoupai = this.mainGroup.getChildByName(name) as HZMJMineShoupai;
		...
		public changeLast2Mopai() {
			if (this.isMopais(this.shoupais.length)) {
				let last = this.getLastMajiang() as HZMJMineShoupai;
