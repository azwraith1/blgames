...
module majiang {
	 ...
	 */
		 ...
		 * 换三张隐藏右边3张牌
		 */
		public hideRightByCount(count: number, isHide: boolean = false) {
			if (count == 0) {
				return;
			}
			if (this.mainGroup.numChildren < 1) {
		...
		public hidePaiByValue(value, isShow) {
			for (var i = 0; i < this.shoupais.length; i++) {
		...
		public removeShoupaiByChi(card = null, maxCard = null) {
			this.removeLastPai();
			this.removeLastPai();
		}
		...
		public showOtherChupaiAni(isBaoTing = false) {
			if (Global.runBack || isBaoTing) {
				this.hideMopai();
				return;
			}
			let card = this.shoupais[Math.floor(_.random(0, this.shoupais.length - 1))];
		...
		public shoupaisVisible() {
			for (let i = 0; i < this.mainGroup.numChildren; i++) {
				let child = this.mainGroup.getChildAt(i);
				child.visible = false;
			}
