module cbzz {
	export class CBZZScrollerItem extends eui.Component {
		private maxSize: number;
		private icons: CBZZIcon[] = [];
		// -1 停止 1 无限循环 0停止
		public runModel: number = 0;
		public stopIcon: CBZZIcon;
		public iconList: CBZZIcon[] = [];
		public index: number;
		...
		public itemDown() {
			...
			for (let i = 0; i < arr.length; i++) {
				...
				if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
					let last = this.findLast() as CBZZIcon;
					// last.changeRamdom();
					icon.y = last.y - 178;
					if (icon.name == "7") {
						this.rollCount += 1;
