...
module bjle {
	export class BjlLd6Panel extends eui.Component {
		...
		public testNums(num, num2, num1) {
			...
			//				5闲赢，6庄对，7闲对，8庄闲对；
			//             9和局 ，10庄对，11闲对，12庄闲对。
			let newList = bjle.BaseBjlLd.changVlue(num.concat([num2]));
		 ...
		 */
		public xianWin(num) {
			let arry = this.testNums(num, 5, 3);
			let arry1 = this.testNums(num, 5, 4);
			let arry2 = this.testNums(num, 5, 5);
