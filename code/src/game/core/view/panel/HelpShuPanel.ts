...
class HelpShuPanel extends game.BaseComponent {
    ...
    private type: string;
    protected HELP_PANEL_NOTIFY: string = PanelNotify.CLOSE_HELP_SHU;
    ...
    public oritationChange(e: egret.Event) {
        var _data = e.data;
        var currentSceneName: string = PanelNotify.OPEN_HELP_SHU;
        var closeName: string = PanelNotify.CLOSE_HELP_SHU;
        //横屏
        if (_data == "H") {
            CF.sN(closeName);
