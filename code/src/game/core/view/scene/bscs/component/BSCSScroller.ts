module bscs {
	export class BSCSScroller extends game.BaseScroller {
		public scroller: eui.Scroller;
		public itemGroup: eui.Group;
		public item1: BSCSScrollerItem;
		public item2: BSCSScrollerItem;
		public item3: BSCSScrollerItem;
		public item4: BSCSScrollerItem;
		public item5: BSCSScrollerItem;
		public aniName = "bscq_reel_fast";
		...
		public constructor() {
			super();
			this.skinName = "BSCSScrollerSkin";
		}
		...
		public update(e: egret.Event) {
			...
			for (let i = 1; i <= 5; i++) {
				let item = this[`item${i}`] as BSCSScrollerItem //.itemDown(this.speed);
				item.itemDown();
			}
		}
		...
		public run() {
			...
			for (let i = 1; i <= 5; i++) {
				let item = this[`item${i}`] as BSCSScrollerItem;
				item.startRun();
			}
		}
		public freeRun() {
		...
		public runResultFast() {
			if (this.resultArrList) {
				...
				for (let i = 0; i < resultArr.length; i++) {
					...
					let item = this[`item${i + 1}`] as BSCSScrollerItem;
		...
		public initItemByFirst(index, firstArr) {
			let item = this[`item${index}`] as BSCSScrollerItem;
