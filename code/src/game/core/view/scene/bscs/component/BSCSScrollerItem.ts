// TypeScript file
module bscs {
    export class BSCSScrollerItem extends eui.Component {
        private maxSize: number;
        private icons: BSCSIcon[] = [];
        // -1 停止 1 无限循环 0停止
        public runModel: number = 0;
        public stopIcon: BSCSIcon;
        public iconList: BSCSIcon[] = [];
        public index: number;
        ...
        public itemDown() {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
                    let last = this.findLast() as BSCSIcon;
                    // last.changeRamdom();
                    icon.y = last.y - 162;
                    if (icon.name == "7") {
                        this.rollCount += 1;
