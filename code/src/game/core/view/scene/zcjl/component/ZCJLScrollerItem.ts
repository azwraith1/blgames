// TypeScript file
module zcjl {
    export class ZCJLScrollerItem extends eui.Component {
        private maxSize: number;
        private icons: ZCJLIcon[] = [];
        // -1 停止 1 无限循环 0停止
        public runModel: number = 0;
        public stopIcon: ZCJLIcon;
        public iconList: ZCJLIcon[] = [];
        public index: number;
        ...
        public itemDown() {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
                    let last = this.findLast() as ZCJLIcon;
                    icon.y = last.y - 362;
                    if (icon.name == "7") {
                        this.rollCount += 1;
                    }
