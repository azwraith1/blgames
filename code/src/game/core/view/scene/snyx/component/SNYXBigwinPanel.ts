module snyx {
    export class SNYXBigwinPanel extends game.BaseComponent {
        ...
        public constructor() {
            super();
            this.skinName = "SNYXBigwinPanelSkin";
        }
        ...
        public showScore(num: number, callback?: Function) {
            ...
            this.timer1.addEventListener(egret.TimerEvent.TIMER, () => {
                if (this.testscore < (game.LaohuUtils.bet * 2) * 30 && this.testscore < num) {
                    this.testscore += index1;
                    //判断是否添加了动画
                    if (!is_big) {
                        this.winImag.source = "snyx_bigwin_1_png";
                    }
                    is_big = true;
                    if (this.testscore) {
                ...
                else if (this.testscore < 50 * (game.LaohuUtils.bet * 2) && this.testscore >= 30 * (game.LaohuUtils.bet * 2) && this.testscore < num) {
                ...
                } else if (this.testscore >= 50 * (game.LaohuUtils.bet * 2) && this.testscore < num) {
                    ...
                    if (num == 30 * (game.LaohuUtils.bet * 2)) {
                        egret.Tween.get(this.winImag).to({ scaleX: 0.2, scaleY: 0.2 }, 20).to({ scaleX: 1, scaleY: 1 }, 200, egret.Ease.sineOut).to({ scaleX: 0.8, scaleY: 0.8 }, 80).to({ scaleX: 1.1, scaleY: 1.1 }, 60).to({ scaleX: 0.9, scaleY: 0.9 }, 60).to({ scaleX: 1, scaleY: 1 }, 50).call(() => {
                            this.winImag.source = RES.getRes("snyx_megawin_1_png");
                        })

                    }
                    if (num == 50 * (game.LaohuUtils.bet * 2)) {
                        egret.Tween.get(this.winImag).to({ scaleX: 0.2, scaleY: 0.2 }, 20).to({ scaleX: 1, scaleY: 1 }, 200, egret.Ease.sineOut).to({ scaleX: 0.8, scaleY: 0.8 }, 80).to({ scaleX: 1.1, scaleY: 1.1 }, 60).to({ scaleX: 0.9, scaleY: 0.9 }, 60).to({ scaleX: 1, scaleY: 1 }, 50).call(() => {
                            this.winImag.source = RES.getRes("snyx_superwin_1_png");
                        })
