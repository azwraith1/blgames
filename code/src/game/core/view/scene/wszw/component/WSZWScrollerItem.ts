// TypeScript file
module wszw {
    export class WSZWScrollerItem extends eui.Component {
        private maxSize: number;
        private icons: WSZWIcon[] = [];
        // -1 停止 1 无限循环 0停止
        public runModel: number = 0;
        public stopIcon: WSZWIcon;
        public iconList: WSZWIcon[] = [];
        public index: number;
        ...
        public itemDown() {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
                    let last = this.findLast() as WSZWIcon;
                    icon.y = last.y - 366;
                    if (icon.name == "7") {
                        this.rollCount += 1;
                    }
