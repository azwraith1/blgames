 ...
 */
module xyxm {
	export class XYXMScrollerItem extends eui.Component {
		private maxSize: number;
		private icons: XYXMIcon[] = [];
		// -1 停止 1 无限循环 0停止
		public runModel: number = 0;
		public stopIcon: XYXMIcon;
		public iconList: XYXMIcon[] = [];
		public index: number;
		...
		public itemDown() {
			...
			for (let i = 0; i < arr.length; i++) {
				...
				if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
					let last = this.findLast() as XYXMIcon;
					// last.changeRamdom();
					icon.y = last.y - 184;
					if (icon.name == "7") {
						this.rollCount += 1;
