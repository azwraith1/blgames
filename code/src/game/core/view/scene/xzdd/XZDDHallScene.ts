...
module majiang {
	export class XZDDHallScene extends BaseMaJiangHallScene {
		public pmdKey: string = "mjxzdd";
		public hallId: string = "mjxzdd";
	...
	// 	public headerFront: string = "hall_header";
		 ...
		 * 关闭当前界面的通知
		 */
		public CLOSE_NOTIFY: string = SceneNotify.CLOSE_MJXZDD_HALL;

		/**
		 * 进入正确匹配的通知
		 */
		public MATCHING_NOTIFY: string = SceneNotify.OPEN_MJXZDD_MATCHING;
	 ...
	 */
		...
		// public showHallBars() {
		// 	var nums = Global.gameProxy.gameNums["mjxzdd"];
		// 	let index = 1;
		// 	var item: any;
		// 	for (let i in nums) {
		// 		let barConfig = nums[i];
		// 		item = new XZDDHallBar(nums[i], index, "mjxzdd");
		// 		item.name = "item" + i;
		// 		item.scaleX = item.scaleY = 1.12;
		// 		this.contentGroup.addChild(item);
		// 		item.x = 60 + item.width / 2 + (index - 1) * (item.width + 70);
		// 		index++;
		// 		item.alpha = 1;
		// 	}
		// }
