 ...
 * @Description: 游戏选择场景。
 */
class HNMJHallScene extends BaseMaJiangHallScene {
	 ...
	 * 关闭当前界面的通知
	 */
	public CLOSE_NOTIFY: string = SceneNotify.CLOSE_HNMJ_HALL;

	/**
	 * 进入正确匹配的通知
	 */
	public MATCHING_NOTIFY: string = SceneNotify.OPEN_HNMJ_MATCHING;
	 ...
	 * 需要加载的资源组
	 */
	public loadGroups: string[] = ['hnmj_game'];
	...
	public async enterScene(e) {
		...
		this.lockEnter = true;
		var data = e.data;
		RotationLoading.instance.load(["hnmj_game"], "", async () => {
	...
	// public showHallBars() {
	// 	var nums = Global.gameProxy.gameNums["hnmj"];
	// 	let index = 1;
	// 	var item: any;
	// 	for (let i in nums) {
	// 		let barConfig = nums[i];
	// 		item = new HNMJHallBar(nums[i], index, "hnmj");
	// 		item.name = "item" + i;
