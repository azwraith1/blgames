// TypeScript file
module csd {
    export class CSDScrollerItem extends eui.Component {
        private maxSize: number;
        private icons: CSDIcon[] = [];
        // -1 停止 1 无限循环 0停止
        public runModel: number = 0;
        public stopIcon: CSDIcon;
        public iconList: CSDIcon[] = [];
        public index: number;
        ...
        public itemDown() {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
                    let last = this.findLast() as CSDIcon;
                    icon.y = last.y - 380;
                    if (icon.name == "7") {
                        this.rollCount += 1;
                    }
