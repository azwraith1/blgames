...
module majiang {
	export class GDMJOverScene extends game.BaseGameScene {
		//下一局
		private playBtn: eui.Button;
		// private backBtn: eui.Button;

		private item1: GDMJOverItemMine;
		private item2: GDMJOverItemOther;
		private item3: GDMJOverItemOther;
		private item4: GDMJOverItemOther;
		...
		public constructor(settleData) {
			...
			this.skinName = "GDMJOverSceneSkin1";
		}
		...
		private showWinPlayers(winPlayerIndex) {
			...
			//创建一个图片一个倍数
			let image = new eui.Image(RES.getRes(`gdmj_over_hutip_${direction}new_png`));
	 ...
	 */
		...
		public createChildren() {
			...
			for (let key in players) {
				...
				if (_direction == "1") {
					if (playerData.gainGold > 0) item.change2Win();
					item.showPlayerBills(playerData.bills, Number(key), this.settleData);
				}
				else {
					item.winBg(playerData.gainGold);
				}
		...
		/**获取总的倍数*/
		private getTotalScore(bills: any): number {
			let infor: any
			let total = 0;
		 ...
		 * 打开游戏界面通知
		 */
		public GAME_SCENE_NOTIFY: string = SceneNotify.CLOSE_GDMJ;
		 ...
		 * 关闭当前界面通知
		 */
		public CLOSE_NOTIFY: string = SceneNotify.CLOSE_GDMJ_OVER;
