 ...
 * @Description: 游戏选择场景。
 */
class GDMJHallScene extends BaseMaJiangHallScene {
	 ...
	 * 关闭当前界面的通知
	 */
	public CLOSE_NOTIFY: string = SceneNotify.CLOSE_GDMJ_HALL;

	/**
	 * 进入正确匹配的通知
	 */
	public MATCHING_NOTIFY: string = SceneNotify.OPEN_GDMJ_MATCHING;
	 ...
	 * 需要加载的资源组
	 */
	public loadGroups: string[] = ['gdmj_game'];
	...
	public async enterScene(e) {
		...
		this.lockEnter = true;
		var data = e.data;
		RotationLoading.instance.load(["gdmj_game"], "", async () => {
	...
	// private createDb() {
	// 	let dbComponent = GameCacheManager.instance.getCache("gdmj_bg") as DBComponent;
	// 	if (!dbComponent) {
	// 		dbComponent = new DBComponent(`gdmj_back`);
	// 		dbComponent.touchEnabled = false;
	// 	}
	// 	dbComponent.playByFilename(0);
	// 	dbComponent.horizontalCenter = -20;
	// 	dbComponent.verticalCenter = 0
	...
	// public showHallBars() {
	// 	var nums = Global.gameProxy.gameNums["gdmj"];
	// 	let index = 1;
	// 	var item: any;
	// 	for (let i in nums) {
	// 		let barConfig = nums[i];
	// 		item = new GDMJHallBar(nums[i], index, "gdmj");
	// 		item.name = "item" + i;
