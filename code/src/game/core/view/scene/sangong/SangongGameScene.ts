module sangong {
	export class SangongGameScene extends game.BaseGameScene {
		 ...
		 * HELP界面
		 */
		public HELP_PANEL_NOTIFY: string = PanelNotify.OPEN_HELP_SHU;
		/**
		 * 记录界面
		 */
		public RECORD_PANEL_NOTIFY: string = PanelNotify.OPEN_NIUGAMERECORD;
		public constructor() {
			super();
		...
		private showStartAni() {
			let startDb = GameCacheManager.instance.getCache("sg_startgame") as DBComponent;
			if (!startDb) {
		...
		/**获取headerInder smart */
		private getHeaderIndex(index): number {
			for (let i = 1; i <= 5; i++) {
				if (this['header' + i].index == index) {
					return i;
		...
		*
		...
		private showGold2Header(index1, index2) {
			...
			let header1 = this['header' + index1] as SangongHeader;
			let header2 = this['header' + index2] as SangongHeader;
			let arr = [];
			for (let i = 0; i < 12; i++) {
				arr.push(i);
			}
			async.eachSeries(arr, (num, callback) => {
				let image = ObjectPool.produce("sangong_coin_img", eui.Image) as eui.Image;
				if (!image) {
				...
				egret.Tween.get(image).wait(15 * (num / 2)).call(() => {//15 * (num / 2) samrt 金币中间等待时间变短
					callback();
				}).to({
		...
		private goldAni(records) {
			...
			async.waterfall([
				(callback) => {
					...
					this.setAutoTimeout(callback, this, 1000);//1500 smart缩短等待时间
				},
				(callback) => {
					//庄家飞闲家
					...
					this.setAutoTimeout(callback, this, 700);//1000
				}
			], (data, callback) => {
				//庄家飞闲家		
		...
		private startMove() {
			...
			async.eachSeries(listArr, (data, callback) => {
				...
				for (let i = data * 5; i < (data + 1) * 5; i++) {
					...
					let time = (150 + (50 * time1));

					egret.Tween.get(poker)
						.to({ verticalCenter: this["pl" + count].verticalCenter, horizontalCenter: this["pl" + count].horizontalCenter }, (150 + (50 * time1)))
					this.setAutoTimeout(() => {
		...
		private oritationChangeLookPais() {
