...
module majiang {
	export class SCMJHallScene extends game.BaseHallScene {
		 ...
		 * 关闭当前界面的通知
		 */
		public CLOSE_NOTIFY: string = SceneNotify.CLOSE_SCMJ_HALL;

		/**
		 * 进入正确匹配的通知
		 */
		public MATCHING_NOTIFY: string = SceneNotify.OPEN_SCMJ_MATCHING;
		...
		public constructor() {
			super();
			this.skinName = `SCMJHallSceneSkin`;
		}
		...
		private choseGameType(gameType) {
			...
			for (let i in Global.gameProxy.gameNums) {
				if (gameType == i) {
					...
					for (let j in games) {
						item = new SCMJHallBar(games[j], gameType);
