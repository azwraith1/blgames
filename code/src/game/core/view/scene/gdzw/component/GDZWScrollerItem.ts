// TypeScript file
module gdzw {
    export class GDZWScrollerItem extends eui.Component {
        private maxSize: number;
        private icons: GDZWIcon[] = [];
        // -1 停止 1 无限循环 0停止
        public runModel: number = 0;
        public stopIcon: GDZWIcon;
        public iconList: GDZWIcon[] = [];
        public index: number;
        ...
        public foGuang4FreeGame(index, str: string) {
            let icons = _.sortBy(this.icons, "y");
            // icons[index + 1].visible = false;
            icons[index + 1].addScatter(str);
        }
        ...
        public itemDown() {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
                    let last = this.findLast() as GDZWIcon;
                    // last.changeRamdom();
                    icon.y = last.y - 189;
                    if (icon.name == "7") {
                        this.rollCount += 1;
