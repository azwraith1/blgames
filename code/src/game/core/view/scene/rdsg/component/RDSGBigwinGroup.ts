// TypeScript file
module rdsg {
    export class RDSGBigwinGroup extends game.BaseComponent {
        public resizeGroup: eui.Group;
        public maskRect: eui.Rect;
        public bigwinbg: eui.Image;
        ...
        public showScore(num: number, callback?: Function) {
            ...
            this.showResultTimeOut = egret.setTimeout(() => {
                this.bigwinbg.alpha = this.winNum.alpha = this.winImag.alpha = 1;
                ...
                this.resizeGroup.addChild(this.bigwinbg);
            ...
            this.timer1.addEventListener(egret.TimerEvent.TIMER, () => {
                if (this.testscore < (game.RDSGUtils.bet * 2) * 30 && this.testscore < num) {
                    this.testscore += index1;
                    //判断是否添加了动画
                    if (!is_big) {
                        this.winImag.source = "rdsg_bigwin_imag_png";
                    }
                    is_big = true;
                    if (this.testscore) {
                ...
                else if (this.testscore < 50 * (game.RDSGUtils.bet * 2) && this.testscore >= 30 * (game.RDSGUtils.bet * 2) && this.testscore < num) {
                ...
                } else if (this.testscore >= 50 * (game.RDSGUtils.bet * 2) && this.testscore < num) {
                    ...
                    if (!is_super) {
                        ...
                        this.titleChangeAni.play("dntg_bigwin_guang", 1);
                        this.titleChangeAni.horizontalCenter = 0;
                        this.titleChangeAni.bottom = 431;
                        this.resizeGroup.addChild(this.titleChangeAni);
                        this.titleChangeAni.resetPosition();
                        SoundManager.getInstance().playEffect("cbzz_winchangemp3_mp3");
                        egret.Tween.get(this.winImag).to({ scaleX: 0.2, scaleY: 0.2 }, 20).to({ scaleX: 1, scaleY: 1 }, 200, egret.Ease.sineOut).to({ scaleX: 0.8, scaleY: 0.8 }, 80).to({ scaleX: 1.1, scaleY: 1.1 }, 60).to({ scaleX: 0.9, scaleY: 0.9 }, 60).to({ scaleX: 1, scaleY: 1 }, 50).call(() => {
                    ...
                    if (num == 30 * (game.RDSGUtils.bet * 2)) {
                        egret.Tween.get(this.winImag).to({ scaleX: 0.2, scaleY: 0.2 }, 20).to({ scaleX: 1, scaleY: 1 }, 200, egret.Ease.sineOut).to({ scaleX: 0.8, scaleY: 0.8 }, 80).to({ scaleX: 1.1, scaleY: 1.1 }, 60).to({ scaleX: 0.9, scaleY: 0.9 }, 60).to({ scaleX: 1, scaleY: 1 }, 50).call(() => {
                            this.winImag.source = RES.getRes("rdsg_megawin_imag_png");
                        })

                    }
                    if (num == 50 * (game.RDSGUtils.bet * 2)) {
                        egret.Tween.get(this.winImag).to({ scaleX: 0.2, scaleY: 0.2 }, 20).to({ scaleX: 1, scaleY: 1 }, 200, egret.Ease.sineOut).to({ scaleX: 0.8, scaleY: 0.8 }, 80).to({ scaleX: 1.1, scaleY: 1.1 }, 60).to({ scaleX: 0.9, scaleY: 0.9 }, 60).to({ scaleX: 1, scaleY: 1 }, 50).call(() => {
                            this.winImag.source = RES.getRes("rdsg_superwin_imag_png");
                        })
        ...
        */
        public removeThisPanel(callback?: Function) {
            ...
            egret.setTimeout(() => {
                ...
                //恢复背景音乐      
                this.maskRect.fillAlpha = 0;
                this.bigwinbg.alpha = this.winNum.alpha = 0;
