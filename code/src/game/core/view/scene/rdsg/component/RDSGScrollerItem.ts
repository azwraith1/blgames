module rdsg {
	export class RDSGScrollerItem extends eui.Component {
		private maxSize: number;
		private icons: RDSGIcon[] = [];
		// -1 停止 1 无限循环 0停止
		public runModel: number = 0;
		public stopIcon: RDSGIcon;
		public iconList: RDSGIcon[] = [];
		public index: number;
		public result: Array<number> = [];
		public scatterAni: DBComponent;
		public downTimeout: egret.Timer;
		...
		public itemDown() {
			...
			for (let i = 0; i < arr.length; i++) {
				...
				if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
					let last = this.findLast() as RDSGIcon;
					// last.changeRamdom();
					icon.y = last.y - 163;
					if (icon.name == "7") {
						this.rollCount += 1;
