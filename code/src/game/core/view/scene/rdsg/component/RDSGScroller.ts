 ...
 */
module rdsg {
	export class RDSGScroller extends game.BaseUI {
		public scroller: eui.Scroller;
		public itemGroup: eui.Group;
		public item1: RDSGScrollerItem;
		public item2: RDSGScrollerItem;
		public item3: RDSGScrollerItem;
		public item4: RDSGScrollerItem;
		public item5: RDSGScrollerItem;
		...
		public constructor() {
			super();
			this.skinName = "RDSGScrollerSkin";
		}
		...
		public update(e: egret.Event) {
			...
			for (let i = 1; i <= 5; i++) {
				let item = this[`item${i}`] as RDSGScrollerItem //.itemDown(this.speed);
				item.itemDown();
			}
		}
		public stopIconDb() {
		...
		public run() {
			...
			for (let i = 1; i <= 5; i++) {
				let item = this[`item${i}`] as RDSGScrollerItem;
				item.startRun();
			}
		}
		public freeRun() {
		...
		public runResultFast() {
			if (this.resultArrList) {
				...
				for (let i = 0; i < resultArr.length; i++) {
					...
					let item = this[`item${i + 1}`] as RDSGScrollerItem;
		...
		public initItemByFirst(index, firstArr) {
			let item = this[`item${index}`] as RDSGScrollerItem;
