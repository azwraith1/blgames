// TypeScript file
module xcbs {
    export class XCBSScrollerItem extends eui.Component {
        private maxSize: number;
        private icons: XCBSIcon[] = [];
        // -1 停止 1 无限循环 0停止
        public runModel: number = 0;
        public stopIcon: XCBSIcon;
        public iconList: XCBSIcon[] = [];
        public index: number;
        ...
        public itemDown() {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (point.y >= 800 && this.runModel == RUN_MODEL.LOOP) {
                    let last = this.findLast() as XCBSIcon;
                    // last.changeRamdom();
                    icon.y = last.y - 180;
                    if (icon.name == "7") {
                        this.rollCount += 1;
        ...
        public updateScrollerItem(index: Array<number>, length, spinResult) {
            ...
            for (let i = 0; i < this.icons.length; i++) {
                ...
                let iconDownY = this.iconDownDis(icon.y);
         ...
         * 图标消除后下降距离
         */
        private iconDownDis(y) {
            let count = 0;
            for (let i = 0; i < this.eliminateIconsY.length; i++) {
                if (y < this.eliminateIconsY[i]) count++;
            }
            return 180 * count;
        }
