...
module game {
    export class UIUtils {
        ...
        */
        ...
        public static addButtonScaleEffects(p: egret.DisplayObjectContainer, soundName = "ui_click_mp3") {
            if (!p) return;
            if (egret.is(p, egret.getQualifiedClassName(eui.Button))) {
                if (!p.name) {
                    p.addEventListener(egret.TouchEvent.TOUCH_BEGIN, () => {
        ...
        public static changeButtonLanguage(button: eui.Button) {
        }


        public static addTouchScaleEffect(p) {
            p.addEventListener(egret.TouchEvent.TOUCH_BEGIN, UIUtils.onButtonTouchBegan, p);
        }

        public static playScaleAni(btn) {
            egret.Tween.get(btn).to({ scaleX: 0.9, scaleY: 0.9 }, 50).to({ scaleX: 1, scaleY: 1 }, 50);
        }

        private static onButtonTouchBegan(btn, soundName) {
        ...
        static setAnchorPotAndScale(p: egret.DisplayObject) {
            if (p.anchorOffsetX || p.anchorOffsetY) {
                return;
            }
            p.anchorOffsetX = p.width / 2 * p.scaleX;
        ...
        public static setGray(p: egret.DisplayObject, flag = true) {
            if (!flag) {
                p.filters = null;
                return;
            }
