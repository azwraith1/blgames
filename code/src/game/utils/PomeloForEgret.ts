 ...
 * @Last Modified time: 2019-05-09 15:48:05
 * @Description: 服务器封装第三方
 */
module PomeloForEgret {
    export class Pomelo {
        static DEBUG: boolean = !true;
        static EVENT_IO_ERROR: string = "io-error";
        static EVENT_CLOSE: string = "pomelo-close";
        static EVEMT_DISCONNECT: string = "disconnect";
        ...
        public init(params, cb: Function): void {
            ...
            // this.heartbeatName = GameConfig.GAME_CONFIG[10006] + ServerConfig.USER_NAME;
            this.handshakeCallback = params.handshakeCallback;
            this.initWebSocket(host, port, cb);
        }
        private initWebSocket(host, port, cb: Function): void {
            LogUtils.logD(`[Pomelo] connect to:${ServerConfig.PATH_CONFIG.socketPath}`)
            ...
            this.socket.connectByUrl(ServerConfig.PATH_CONFIG.socketPath);
        }
        ...
        private onConnect(e: egret.Event): void {
            LogUtils.logD("[Pomelo] connect success", e);
            if (this.socket) {
                this.socket.addEventListener(egret.Event.CLOSE, this.onClose, this);
                this.socket.addEventListener(egret.IOErrorEvent.IO_ERROR, this.onIOError, this);
        ...
        private onClose(e: egret.Event): void {
            console.log("[Pomelo] connect close:", e);
            this.emit(Pomelo.EVENT_CLOSE, e);
            this.emit(Pomelo.EVEMT_DISCONNECT, e);
            CF.dP(Pomelo.EVENT_CLOSE, e);
            CF.dP(Pomelo.EVEMT_DISCONNECT, e);
            if (this.socket) {
        ...
        private onIOError(e: egret.Event): void {
            LogUtils.logD("[Pomelo] connect close:", e);
            // this.emit(Pomelo.EVENT_IO_ERROR, e);
            CF.dP(Pomelo.EVENT_IO_ERROR, e);
            // egret.error('socket error: ', e);
        }
        ...
        private heartbeatTimeoutCb(data) {
            ...
            if (gap > this.gapThreshold) {
                this.heartbeatTimeoutId = setTimeout(self.heartbeatTimeoutCb.bind(self), gap);
            } else {
                this.emit(Pomelo.EVENT_HEART_BEAT_TIMEOUT, data);
                this.emit(Pomelo.EVEMT_DISCONNECT, data);
                this._disconnect();
            }
        }
        public off(event?, fn?) {
        ...
        private emit(event, ...args: any[]) {
            ...
            if (callbacks) {
                ...
                for (var i = 0, len = callbacks.length; i < len; ++i) {
                    callbacks[i].apply(this, params, event);
                }
            }
    ...
    class Package implements IPackage {
        ...
        public decode(buffer: egret.ByteArray) {
            ...
            if (buffer.bytesAvailable >= len) {
                ...
                LogUtils.logI("[Package] no enough length for current type:" + type);
            }
